        ...
        error:function(file,response){
           console.log(response);
            ...
            if(response.errors != undefined){
                ...
                $('.errorsImage').append('<div class="text-danger mt-2 ml-4"><i class="fas fa-exclamation"></i> <span class="font600">'+lang[langN]['Error_subir']+': "'+file.name+'":</span> </div><ul class="text-danger ml-4">'+lista+'</ul>');
            }else{
                $('.errorsImage').append('<div class="text-danger mt-2 ml-4"><i class="fas fa-exclamation"></i> <span class="font600">'+lang[langN]['Error_subir']+': "'+file.name+'"</span> </div>');
            }
        ...
        success:function(file,response){
            ...
            if (response.result == 'error') {
               complete = 'error';
                ...
                $('.cardErrorsImages').show();
                warning(lang[langN]['Error_subir']+': '+file.name);
            }else{
               parent = $(this.element).parents('.card').find('.card-header').find('.images_load');
